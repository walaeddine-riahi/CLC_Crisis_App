(() => {
  ...
  function renderAdminDashboard() {
    ...
    const additionalAccess = user => {
      ...
      const entities = [...new Set(actionCatalog.map(item => item.entity))];
      const owners = [...new Set(actionCatalog.map(item => item.owner))].sort((a, b) => a.localeCompare(b, 'fr'));
  ...
  function bindAdminDashboard() {
    ...
    const filterAccessPanel = panel => {
      ...
      const search = (panel.querySelector('[data-access-filter="search"]')?.value || '').trim().toLocaleLowerCase('fr');
      panel.querySelectorAll('[data-access-item]').forEach(item => {
        const matches = (!entity || item.dataset.entity === entity) && (!owner || item.dataset.owner === owner) && (!search || (item.dataset.label || '').toLocaleLowerCase('fr').includes(search));
        item.hidden = !matches;
      });
    };
    const updateAccessCount = panel => { const count = panel.querySelectorAll('[data-action-access]:checked').length, target = panel.querySelector('[data-access-count]'); if (target) target.textContent = `${count} autorisation(s)`; };
    els.adminHost.querySelectorAll('[data-access-toggle]').forEach(button => button.addEventListener('click', () => {
      const card = button.closest('[data-admin-user]'), panel = card?.querySelector('[data-access-panel]'); if (!panel) return;
      panel.hidden = !panel.hidden; if (!panel.hidden) { filterAccessPanel(panel); panel.scrollIntoView({ behavior: 'smooth', block: 'nearest' }); }
    }));
    els.adminHost.querySelectorAll('[data-access-panel]').forEach(panel => {
      panel.querySelectorAll('[data-access-filter]').forEach(filter => filter.addEventListener(filter.tagName === 'INPUT' ? 'input' : 'change', () => filterAccessPanel(panel)));
      panel.querySelectorAll('[data-action-access]').forEach(input => input.addEventListener('change', () => updateAccessCount(panel)));
      panel.querySelector('[data-access-select-visible]')?.addEventListener('click', () => { panel.querySelectorAll('[data-access-item]:not([hidden]) [data-action-access]').forEach(input => { input.checked = true; }); updateAccessCount(panel); });
      panel.querySelector('[data-access-clear-visible]')?.addEventListener('click', () => { panel.querySelectorAll('[data-access-item]:not([hidden]) [data-action-access]').forEach(input => { input.checked = false; }); updateAccessCount(panel); });
      panel.querySelector('[data-access-close]')?.addEventListener('click', () => { panel.hidden = true; });
      panel.querySelector('[data-access-save]')?.addEventListener('click', async () => {
        const card = panel.closest('[data-admin-user]'), id = card?.dataset.adminUser; if (!id) return;
        const actionAccess = [...panel.querySelectorAll('[data-action-access]:checked')].map(input => input.value);
        try { await api('/api/admin/users', { method: 'PATCH', body: JSON.stringify({ id, actionAccess }) }); adminState.message = 'Accès aux actions enregistrés.'; adminState.messageType = 'success'; await loadAdminDashboard(adminState.pagination.page || 1); }
        catch (error) { adminMessage(error.message, 'error'); }
      });
    });
