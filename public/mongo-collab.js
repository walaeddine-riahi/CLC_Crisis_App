(() => {
  ...
  let adminState = { users: [], roles: [], currentUserId: null, logs: [], pagination: { page: 1, pages: 1, total: 0 }, stats: { total: 0, last24h: 0, activeUsers24h: 0 }, filters: { eventType: '', search: '' }, loading: false, message: '', messageType: '' };
  ...
  function renderAdminDashboard() {
    ...
    const roleOptions = (adminState.roles || Object.keys(ROLE_LABELS)).map(r => `<option value="${escapeHtml(r)}"${r === 'VIEWER' ? ' selected' : ''}>${escapeHtml(ROLE_LABELS[r] || r)}</option>`).join('');
  ...
  async function loadAdminDashboard(page = 1) {
    ...
    try {
      ...
      const [userData, logData] = await Promise.all([api('/api/admin/users'), api(`/api/admin/logs?${query}`)]);
      adminState = { ...adminState, ...userData, logs: logData.logs || [], pagination: logData.pagination, stats: logData.stats, loading: false };
    } catch (error) { adminState.loading = false; adminState.message = error.message; adminState.messageType = 'error'; }
  ...
  function bindAdminDashboard() {
    ...
    els.adminHost.querySelectorAll('[data-admin-action]').forEach(button => button.addEventListener('click', async () => {
      ...
      try {
        if (button.dataset.adminAction === 'save') await api('/api/admin/users', { method: 'PATCH', body: JSON.stringify({ id, displayName: card.querySelector('[data-field="displayName"]').value, role: card.querySelector('[data-field="role"]').value, entity: card.querySelector('[data-field="entity"]').value }) });
